...
class GeneticEmbedder(BaseEmbedder):
    """
    Generates 32-dimensional Genetic embeddings.
    """
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        patnos = X.index
        # Explicit bypass features + padding
        vals = X.values
        ...
        return pd.DataFrame(padded, index=patnos, columns=[f"Gen_{i}" for i in range(self._output_dim)])
